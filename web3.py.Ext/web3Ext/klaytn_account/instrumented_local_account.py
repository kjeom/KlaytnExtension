...
def klaytn_extended_recover_transaction_as_feepayer(self, serialized_transaction):
    ...
    typed_transaction = TypedTransaction.from_dict({'type':txn_bytes[0]})
    typed_transaction = typed_transaction.from_bytes(txn_bytes)
    msg_hash = typed_transaction.transaction.feepayer_hash()
    vrs = typed_transaction.transaction.feepayer_vrs()
